...
if (!class_exists('ListingoAds_Widget')) {

    class ListingoAds_Widget extends WP_Widget {
        /*
        ...
        public function __construct() {
            $widget_ops = array('classname' => 'address-column', 'description' => esc_html__('Display Ads Widget', 'listingo_core'));
            $control_ops = array('width' => 300, 'height' => 250, 'id_base' => 'ads_widget');
            parent::__construct('ads_widget', esc_html__('Ads Script | Listingo', 'listingo_core'), $widget_ops, $control_ops);
        }
        ...
        public function form($instance) {
		  ...
		  ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'listingo_core'); ?></label>
                <input type="text" id="title" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widefat" />
            </p>
			<p>
                <label for="<?php echo esc_attr($this->get_field_id('add_code')); ?>"><?php esc_html_e('Add Script', 'listingo_core'); ?></label>
...
add_action('widgets_init', create_function('', 'return register_widget("ListingoAds_Widget");'));