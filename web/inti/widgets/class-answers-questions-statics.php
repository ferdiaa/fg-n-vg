...
if (!class_exists('Listingo_AQ_Statics')) {

    class Listingo_AQ_Statics extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct(
                    'listingo_aq_staticcs' , // Base ID
                    esc_html__('Answers & Questions Statics | Listingo' , 'listingo_core') , // Name
                array (
                	'classname' => 'tg-burfats questions-statics-widgets',
					'description' => esc_html__('Answers & Questions Statics' , 'listingo_core') , 
				) // Args
            );
        }
			...
			if (function_exists('fw_get_db_settings_option') && fw_ext('questionsanswers')) {
				...
				<div class="aq-statics">
					<ul class="tg-votesanswers">
						<li><a><em><?php  esc_html_e('Total Questions posted' , 'listingo_core');?></em> <span><?php echo intval($total_questions); ?></span></a></li>
						<li><a><em><?php  esc_html_e('Total Queries answered' , 'listingo_core');?></em> <span><?php echo intval($question_total_ans); ?></span></a></li>
					</ul>
				</div>
				<?php
			} else{?>
        ...
        public function form($instance) {
			// outputs the options form on admin
			$title           = !empty($instance['title']) ? $instance['title'] : esc_html__('Statics' , 'listingo_core');
			?>
			<p>
...
//register widget
add_action('widgets_init', create_function('', 'return register_widget("Listingo_AQ_Statics");'));