...
class Envato_marketplaces {
   ...
   */
   public function display_thumbs($user_name, $marketplace_name, $limit = null)
   {
      ...
      <li>
          <a href="<?php echo esc_url( $item->url ) . "?ref=$user_name"; ?>" title="<?php echo esc_attr( $item->item );?>">   
             <img src="<?php echo esc_url( $item->thumbnail ); ?>" alt="<?php echo esc_attr( $item->item )?>">
          </a>
      </li>
      <?php endforeach;
      echo "\n</ul>";
         ...
         $data = $this->curl($url);
		  if ($data) {
            $data = isset($set) ? $data->{$set} : $data; // if a set is needed, update
			  ...
			  return esc_html__('Could not retrieve data.','listingo_core');
		  } 
