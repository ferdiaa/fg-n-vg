 ...
 * @return 
 */
require ListingoGlobalSettings::get_plugin_path() . 'libraries/Facebook/autoload.php';
...
class Facebook_Connect {
    ...
    public function __construct() {
		...
		add_action('wp_ajax_listingo_fb_connect', array(&$this, 'get_login_url'));
		add_action('wp_ajax_nopriv_listingo_fb_connect', array(&$this, 'get_login_url'));
    }
    ...
    private function initApi() {
		if ( function_exists( 'fw_get_db_settings_option' ) ) {
			...
			$this->callback_url = listingo_new_social_login_url( 'facebooklogin' );
		}
    ...
    public function do_facebook_connect() {
		...
		$redirectURL = listingo_new_social_login_url( 'facebooklogin' );
		...
		if (isset($accessToken)) {
			...
			try {
				...
				set_site_transient( listingo_get_uniqid() . '_sp_facebook_connect', $accessToken, 3600 );
				$email = filter_var( $user[ 'email' ], FILTER_SANITIZE_EMAIL );

				if ( !is_user_logged_in() ) {
					$ID = email_exists( $email );
					if ( $ID == false ) { // Real register
						do_action('listingo_create_social_users','facebook',$user);
					} else if ( $ID ) { // Login
	...
	public function get_login_url() {
		...
		$json['message'] 		= esc_html__('Please wait while you are redirecting to facebook for authorizations.', 'listingo_core');
