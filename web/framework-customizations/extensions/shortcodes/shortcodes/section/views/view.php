...
if (isset($atts['parallax']) && $atts['parallax'] === 'on' && isset($atts['background_image']['data']['icon']) && !empty($atts['background_image']['data']['icon'])) {
	...
	$parallax_url	= listingo_add_http_protcol($parallax_url);
...
if( isset( $atts['is_fullwidth'] ) && $atts['is_fullwidth'] === 'stretch_section' ){
	$section_extra_classes[] = 'stretch_section service-providers-elm-section';
	...
	$section_extra_classes[] = 'stretch_section_contents stretch_section stretch_data service-providers-elm-section';
	$full_width_section	= '<div class="section-current-width"></div>';
} else if( isset( $atts['is_fullwidth'] ) && $atts['is_fullwidth'] === 'stretch_section_contents_corner' ){
	$section_extra_classes[] = 'stretch_section_contents_corner stretch_section stretch_data service-providers-elm-section';
	$full_width_section	= '<div class="section-current-width"></div>';
} else{
	$section_extra_classes[] = 'default service-providers-elm-section';
	$full_width_section	= '';
}
...
<?php } else{?>
    <section  class="tg-main-section tg-haslayout <?php echo implode(' ',$section_extra_classes ) ?>" <?php echo ($custom_id); ?> <?php echo ($section_style); ?> <?php echo ($bg_video_data_attr); ?> <?php echo ($paralaxx_configuration); ?>>
        <div class="builder-items">
            <?php if (isset($atts['section_heading']) && !empty($atts['section_heading'])) { ?>
                <h2 class="tg-section-title"><?php echo esc_attr($atts['section_heading']); ?></h2>
            <?php } ?>
