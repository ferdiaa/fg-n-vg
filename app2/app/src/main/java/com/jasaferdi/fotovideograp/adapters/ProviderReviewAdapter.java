...
public class ProviderReviewAdapter extends RecyclerView.Adapter<ProviderReviewAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if(mValues.get(position).getTotalRatings() != null) {
            holder.ratingBar.setRating(Integer.parseInt(mValues.get(position).getTotalRatings()));
        }
        ...
        Picasso.with(holder.thumbnail.getContext()).load(mValues.get(position).getAvatar()).into(holder.thumbnail);
