...
public class RejectAppointmentActivity2 extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_reject_appointment2);

        getSupportActionBar().setTitle(R.string.reject_appointment);

        title = findViewById(R.id.rejection_title2);
        description = findViewById(R.id.rejection_description2);

        final Appointment apt = (Appointment) getIntent().
                getBundleExtra(Constants.DATA).getSerializable(Constants.DATA);

        findViewById(R.id.reject_now2).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ...
                RetrofitUtil.createProviderAPI().updateProviderAppointments
                        (req).enqueue(RetrofitUtil.sendRequest(RejectAppointmentActivity2.this));
